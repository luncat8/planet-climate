...
var PARAMS = {
  ...
  /* ---- UNIFIED CRYOSPHERE (one "ice" type over sea & land) ------------------
     Single ice field (iceThk [m] + iceFrac [0..1]) advected/driven over water
     and creeping/calving over land. Full climate feedback (albedo, insulation,
     latent heat, brine) is applied in COUPLE_FS; horizontal dynamics in ICE_DYN_FS. */
  iceOn:      { label: 'Ice enabled',          default: 1, min: 0, max: 1, step: 1,
    fmt: function (v) { return v > 0.5 ? 'on' : 'off'; },
    tip: 'Master switch for the unified ice model (sea + land).' },
  iceLf:      { label: 'Latent heat',          default: 3.34e5, min: 1e5, max: 5e5, step: 1e4,
    fmt: function (v) { return v.toExponential(1); } },
  iceRho:     { label: 'Ice density',          default: 917, min: 500, max: 1000, step: 5,
    fmt: function (v) { return v.toFixed(0) + ' kg/m³'; } },
  iceGrowth:  { label: 'Thermo growth',        default: 4e-7, min: 0, max: 1e-5, step: 1e-8,
    fmt: function (v) { return v.toExponential(1); },
    tip: 'Ice growth/melt rate (m/s per K of super-cooling). Sets the e-folding timescale of freeze/melt.' },
  insulK:     { label: 'Ice insulation',       default: 0.7, min: 0, max: 0.95, step: 0.02,
    fmt: function (v) { return v.toFixed(2); },
    tip: 'How strongly a full ice cover suppresses air-sea heat & moisture exchange.' },
  albIceSea:  { label: 'Sea-ice albedo+',      default: 0.45, min: 0, max: 0.6, step: 0.02,
    fmt: function (v) { return v.toFixed(2); } },
  albIceLand: { label: 'Land-ice albedo+',     default: 0.55, min: 0, max: 0.7, step: 0.02,
    fmt: function (v) { return v.toFixed(2); } },
  precipIce:  { label: 'Cold-rain→ice',       default: 0.6, min: 0, max: 1, step: 0.05,
    fmt: function (v) { return v.toFixed(2); },
    tip: 'Fraction of precipitation that freezes onto land ice (instead of running off) when below freezing.' },
  maxIce:     { label: 'Max ice thickness',    default: 60, min: 5, max: 200, step: 5,
    fmt: function (v) { return v.toFixed(0) + ' m'; } },
  iceFull:    { label: 'Ice full coverage',    default: 1.5, min: 0.2, max: 5, step: 0.1,
    fmt: function (v) { return v.toFixed(1) + ' m'; },
    tip: 'Ice thickness that gives 100% sub-grid coverage (iceFrac=1).' },
  windIce:    { label: 'Sea-ice wind drift',   default: 0.02, min: 0, max: 0.1, step: 0.002,
    fmt: function (v) { return v.toFixed(3); },
    tip: 'Sea-ice drift coefficient: ice velocity = u_top + windIce * wind.' },
  iceCreep:   { label: 'Land-ice creep',       default: 0.5, min: 0, max: 5, step: 0.1,
    fmt: function (v) { return v.toFixed(2); },
    tip: 'Land-ice downslope creep rate (m²/s per metre of ice-surface slope).' },
  calve:      { label: 'Calving rate',         default: 0.01, min: 0, max: 0.1, step: 0.002,
    fmt: function (v) { return v.toFixed(3); },
    tip: 'Fraction of excess land-ice thickness that calves into the sea per step at the coast.' },
  calveThk:   { label: 'Calving thickness',    default: 20, min: 0, max: 100, step: 5,
    fmt: function (v) { return v.toFixed(0) + ' m'; } },
  iceCFL:     { label: 'Ice CFL clamp',       default: 5, min: 1, max: 20, step: 1,
