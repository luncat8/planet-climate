...
var PARAMS = {
  ...
  flowGain:       { label: 'Flow gain', default: 1, min: 0.2, max: 8, step: 0.1,
                    fmt: function (v) { return (+v).toFixed(1) + '×'; } },
  // ---- Cryosphere (sea ice + land snow/glaciers). See CRYOSPHERE.md. ----
  iceOn:        { label: 'Sea ice & snow', default: 1 },
  iceOverlay:   { label: 'Ice overlay on map', default: 1 },
  iceAlbedo:    { label: 'Sea-ice albedo', default: 0.62, min: 0.3, max: 0.85, step: 0.01 },
  snowAlbedo:   { label: 'Land snow albedo', default: 0.78, min: 0.4, max: 0.9, step: 0.01 },
  iceInsul:     { label: 'Ice insulation', default: 0.85, min: 0, max: 0.98, step: 0.01,
                  fmt: function (v) { return Math.round(v * 100) + '%'; } },
  iceH0:        { label: 'Ice coverage scale', default: 0.5, min: 0.1, max: 3, step: 0.1,
                  fmt: function (v) { return (+v).toFixed(1) + ' m'; } },
  iceSalinity:  { label: 'Sea-ice salinity', default: 4, min: 0, max: 15, step: 0.5,
                  fmt: function (v) { return (+v).toFixed(1) + ' ppt'; } },
  iceFreezeRate:{ label: 'Freeze/melt rate', default: 5e-5, min: 5e-6, max: 1e-3, step: 5e-6,
                  fmt: function (v) { return (+v).toExponential(1) + '/s'; } },
  iceDriftWind: { label: 'Ice wind factor', default: 0.02, min: 0, max: 0.05, step: 0.005,
                  fmt: function (v) { return Math.round(v * 100) + '%'; } },
  iceDriftCurrent: { label: 'Ice current factor', default: 1.0, min: 0, max: 1.5, step: 0.05,
                  fmt: function (v) { return (+v).toFixed(2) + '×'; } },
  iceSia:       { label: 'Glacial flow rate', default: 3e-8, min: 0, max: 3e-7, step: 1e-8,
                  fmt: function (v) { return (+v).toExponential(1); } },
